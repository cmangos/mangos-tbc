...
bool Creature::IsImmuneToSpell(SpellEntry const* spellInfo, bool castOnSelf, uint8 effectMask, WorldObject const* caster)
{
    ...
    if (!castOnSelf)
    {
        if (GetCreatureInfo()->MechanicImmuneMask & getBitmask(spellInfo->Mechanic))
            return true;
...
{
    if (!castOnSelf && GetCreatureInfo()->MechanicImmuneMask & getBitmask(spellInfo->EffectMechanic[index]))
        return true;
