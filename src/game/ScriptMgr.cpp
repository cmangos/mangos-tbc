...
{
    // used by eluna
    if (pCreature && pCreature->FindMap())
        if (CreatureAI* luaAI = pCreature->FindMap()->GetEluna()->GetAI(pCreature))
            return luaAI;
...
{
    if (ElunaIf(pPlayer, OnGossipHello(pPlayer, pCreature)))
        return true;
...
{
    if (ElunaIf(pPlayer, OnGossipHello(pPlayer, pGameObject)))
        return true;
...
bool ScriptMgr::OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 sender, uint32 action, const char* code)
{
    if (code)
    {
        if (ElunaIf(pPlayer, OnGossipSelectCode(pPlayer, pCreature, sender, action, code)))
            return true;
    }
    else
    {
        if (ElunaIf(pPlayer, OnGossipSelect(pPlayer, pCreature, sender, action)))
            return true;
    }
...
bool ScriptMgr::OnGossipSelect(Player* pPlayer, GameObject* pGameObject, uint32 sender, uint32 action, const char* code)
{
    if (code)
    {
        if (ElunaIf(pPlayer, OnGossipSelectCode(pPlayer, pGameObject, sender, action, code)))
            return true;
    }
    else
    {
        if (ElunaIf(pPlayer, OnGossipSelect(pPlayer, pGameObject, sender, action)))
            return true;
    }
...
{
    if (ElunaIf(pPlayer, OnQuestAccept(pPlayer, pCreature, pQuest)))
        return true;
...
{
    if (ElunaIf(pPlayer, OnQuestAccept(pPlayer, pGameObject, pQuest)))
        return true;
...
{
    if (ElunaIf(pPlayer, OnQuestAccept(pPlayer, pItem, pQuest)))
        return true;
...
{
    // used by eluna
    if (pPlayer && pPlayer->FindMap())
        if (uint32 dialogId = pPlayer->FindMap()->GetEluna()->GetDialogStatus(pPlayer, pCreature))
            return dialogId;
...
uint32 ScriptMgr::GetDialogStatus(Player* pPlayer, GameObject* pGameObject)
{
    // used by eluna
    if (pPlayer && pPlayer->FindMap())
        if (uint32 dialogId = pPlayer->FindMap()->GetEluna()->GetDialogStatus(pPlayer, pGameObject))
            return dialogId;
...
{
    if (ElunaIf(pPlayer, OnGameObjectUse(pPlayer, pGameObject)))
        return true;
...
{
    if (!ElunaIf(pPlayer, OnUse(pPlayer, pItem, targets)))
        return true;
...
{
    if (ElunaIf(pPlayer, OnAreaTrigger(pPlayer, atEntry)))
        return true;
...
{
    if (ElunaIf(pCaster, OnDummyEffect(pCaster, spellId, effIndex, pTarget)))
        return true;
