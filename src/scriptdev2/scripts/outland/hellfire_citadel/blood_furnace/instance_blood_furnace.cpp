...
instance_blood_furnace::instance_blood_furnace(Map* pMap) : ScriptedInstance(pMap),
    m_uiBroggokEventTimer(pMap->IsRegularDifficulty() ? 90 * IN_MILLISECONDS : 30 * IN_MILLISECONDS),
...
void instance_blood_furnace::DoNextBroggokEventPhase()
{
    ...
    m_uiBroggokEventTimer = (instance->IsRegularDifficulty() ? 90 * IN_MILLISECONDS : 30 * IN_MILLISECONDS);
...
void instance_blood_furnace::Update(uint32 uiDiff)
{
    ...
    if (m_auiEncounter[TYPE_BROGGOK_EVENT] == IN_PROGRESS && m_uiBroggokEventPhase < MAX_ORC_WAVES)
    {
