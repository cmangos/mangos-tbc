...
void Unit::CalculateMeleeDamage(Unit* pVictim, CalcDamageInfo* damageInfo, WeaponAttackType attackType /*= BASE_ATTACK*/)
{
    ...
    for (uint8 i = 0; i < m_weaponDamageCount[damageInfo->attackType]; i++)
    {
        ...
        subDamage->damageSchoolMask = SpellSchoolMask(1 << GetWeaponDamageSchool(damageInfo->attackType, i));
    ...
    switch (damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_CRIT:
        {
            ...
            mod += GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_CRIT_PERCENT_VERSUS, creatureTypeMask);
            ...
            if (pVictim->GetTypeId() == TYPEID_PLAYER)
            {
                uint32 resilienceReduction = ((Player*)pVictim)->GetMeleeCritDamageReduction(damageInfo->totalDamage);
                damageInfo->totalDamage -= resilienceReduction;
...
SpellMissInfo Unit::SpellHitResult(Unit* pVictim, SpellEntry const* spell, bool CanReflect)
{
    ...
    if (wand && !!(getClassMask() & CLASSMASK_WAND_USERS) && GetTypeId() == TYPEID_PLAYER)
        schoolMask = SpellSchoolMask(1 <<GetWeaponDamageSchool(RANGED_ATTACK));
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    for (uint8 i = 0; i < m_weaponDamageCount[damageInfo->attackType]; i++)
    {
        ...
        data << uint32(GetFirstSchoolInMask(subDamage->damageSchoolMask));  // School of sub damage
        ...
        data << uint32(subDamage->absorb);                                  // Absorb
        data << uint32(subDamage->resist);                                  // Resist
    }
    //=================================================
