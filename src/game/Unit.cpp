...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    if (health <= damage)
    {
        ...
        if (player_tap != pVictim)
        {
            bool xpkill = false;
            if (group_tap)
                xpkill = group_tap->RewardGroupAtKill(pVictim, player_tap);
            else if (player_tap)
                xpkill = player_tap->RewardSinglePlayerAtKill(pVictim);

            if (player_tap && xpkill)
                player_tap->ProcDamageAndSpell(pVictim,PROC_FLAG_KILL_XP_GIVER,PROC_FLAG_NONE);
        }
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags,WeaponAttackType attackType, uint32 cooldown)
{
    ...
    switch(auraSpellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if(auraSpellInfo->SpellFamilyFlags & UI64LIT(0x0000000000004000))
            {
                ...
                for(Unit::AuraList::const_iterator i = mAddFlatModifier.begin(); i != mAddFlatModifier.end(); ++i)
                {
                    ...
                    {
                        int32 value2 = CalculateSpellDamage(this,(*i)->GetSpellProto(),EFFECT_INDEX_2);
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if((auraSpellInfo->SpellFamilyFlags & UI64LIT(0x80000000)) && auraSpellInfo->SpellVisual==7958)
            {
                ...
                switch(triggeredByAura->GetSpellProto()->Id)
                {
                    ...
                    case 25477:
                        trigger_spell_id = 28385; break;    // Rank 7
                    default:
            ...
            switch(auraSpellInfo->Id)
            {
                ...
                case 37336:
                {
                    switch(m_form)
                    {
                        case FORM_BEAR:
                        case FORM_DIREBEAR:
                            trigger_spell_id = 37340; break;// Ursine Blessing
                        case FORM_CAT:
                            trigger_spell_id = 37341; break;// Feline Blessing
                        case FORM_TREE:
                            trigger_spell_id = 37342; break;// Slyvan Blessing
                        case FORM_MOONKIN:
                            trigger_spell_id = 37343; break;// Lunar Blessing
                        case FORM_NONE:
                            trigger_spell_id = 37344; break;// Cenarion Blessing (for caster form, except FORM_MOONKIN)
                        default:
                            return false;
                    }
        ...
        case SPELLFAMILY_SHAMAN:
        {
            if(auraSpellInfo->SpellFamilyFlags == UI64LIT(0x0000000000000000))
            {
                switch(auraSpellInfo->SpellIconID)
                {
                    ...
                    case 2013:
                    {
                        ...
                        if (10*(int32(GetHealth())-int32(damage)) >= 3*int32(GetMaxHealth()))
                            return false;
            ...
            if((auraSpellInfo->SpellFamilyFlags & UI64LIT(0x0000002000000000)) && auraSpellInfo->SpellVisual==7358)
            {
                target = this;
                break;
            }
            ...
            if((auraSpellInfo->SpellFamilyFlags & UI64LIT(0x00000400)) && auraSpellInfo->SpellVisual==37)
            {
                ...
                switch(triggeredByAura->GetSpellProto()->Id)
                {
                    ...
                    case 25469:
                        trigger_spell_id = 26371; break;    // Rank 8
                    case 25472:
                        trigger_spell_id = 26372; break;    // Rank 9
                    default:
    ...
    for(AuraTypeSet::const_iterator aur = procAuraTypes.begin(); aur != procAuraTypes.end(); ++aur)
    {
        ...
        for(ProcTriggeredList::iterator i = procTriggered.begin(); i != procTriggered.end(); ++i)
        {
            ...
            switch(*aur)
            {
                ...
                {
                    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "ProcDamageAndSpell: casting spell (triggered by %s proc aura of spell %u)",
                ...
                case SPELL_AURA_PROC_TRIGGER_DAMAGE:
                {
                    ...
                    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "ProcDamageAndSpell: doing %u damage (triggered by %s aura of spell %u)",
                ...
                case SPELL_AURA_MANA_SHIELD:
                {
                    ...
                    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "ProcDamageAndSpell: casting spell (triggered by %s dummy aura of spell %u)",
                        (isVictim?"a victim's":"an attacker's"),triggeredByAura->GetId());
                ...
                {
                    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "ProcDamageAndSpell: casting spell (triggered by %s haste aura of spell %u)",
                        (isVictim?"a victim's":"an attacker's"),triggeredByAura->GetId());
                ...
                {
                    DEBUG_FILTER_LOG(LOG_FILTER_SPELL_CAST, "ProcDamageAndSpell: casting spell (triggered by %s class script aura of spell %u)",
                        (isVictim?"a victim's":"an attacker's"),triggeredByAura->GetId());
