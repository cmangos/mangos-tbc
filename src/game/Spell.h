...
class Spell
{
    ...
    public:
        ...
        void DoSummonWild(SpellEffectIndex eff_idx);
        void DoSummonGuardian(SpellEffectIndex eff_idx);
        void DoSummonTotem(SpellEffectIndex eff_idx);
        void DoSummonCritter(SpellEffectIndex eff_idx);
        void DoSummonDemon(SpellEffectIndex eff_idx);
    ...
    struct MANGOS_DLL_DECL SpellNotifierCreatureAndPlayer
    {
        ...
        template<class T> inline void Visit(GridRefManager<T>  &m)
        {
            ...
            for(typename GridRefManager<T>::iterator itr = m.begin(); itr != m.end(); ++itr)
            {
                ...
                {
                    case PUSH_IN_FRONT:
                        if(i_spell.GetCaster()->isInFrontInMap((Unit*)(itr->getSource()), i_radius, 2*M_PI_F/3 ))
                            i_data->push_back(itr->getSource());
                        break;
                    case PUSH_IN_FRONT_90:
                        if(i_spell.GetCaster()->isInFrontInMap((Unit*)(itr->getSource()), i_radius, M_PI_F/2 ))
                            i_data->push_back(itr->getSource());
                        break;
                    case PUSH_IN_FRONT_15:
                        if(i_spell.GetCaster()->isInFrontInMap((Unit*)(itr->getSource()), i_radius, M_PI_F/12 ))
                            i_data->push_back(itr->getSource());
                        break;
                    case PUSH_IN_BACK:
                        if(i_spell.GetCaster()->isInBackInMap((Unit*)(itr->getSource()), i_radius, 2*M_PI_F/3 ))
                            i_data->push_back(itr->getSource());
                        break;
