...
struct VendorItem
{
    VendorItem(uint32 _item, uint32 _maxcount, uint32 _incrtime, uint32 _ExtendedCost, uint16 _conditionId, uint32 _currencyId)
        : item(_item), maxcount(_maxcount), incrtime(_incrtime), ExtendedCost(_ExtendedCost), conditionId(_conditionId), currencyId(_currencyId) {}
...
struct VendorItemData
{
    ...
    void AddItem(uint32 item, uint32 maxcount, uint32 ptime, uint32 ExtendedCost, uint16 conditonId, uint32 currencyId)
    {
        m_items.push_back(new VendorItem(item, maxcount, ptime, ExtendedCost, conditonId, currencyId));
    }
