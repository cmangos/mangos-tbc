...
void Spell::EffectSummonPet(SpellEffectIndex eff_idx)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        else if (m_caster->getClass() == CLASS_MAGE)
        {
            int32 duration = CalculateSpellDuration(m_spellInfo, m_caster);

            if (m_duration > 0)
                NewSummon->SetDuration(m_duration);
        }
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
			...
			case 32027:
			{
				...
				Creature* pCreature = m_caster->SummonCreature(creature_id, 0.0f, 0.0f, 0.0f, angle, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 60000, true);

				if (pCreature->AI())
					pCreature->AI()->AttackStart(m_caster->getVictim());
				return;
