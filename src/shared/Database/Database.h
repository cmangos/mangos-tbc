...
class Database
{
    public:
        ...
        void AllowAsyncTransactions() { mAllowAsyncTransactions = true; }

    protected:
        Database() :
            m_nQueryConnPoolSize(1), m_pAsyncConn(nullptr), m_pResultQueue(nullptr),
            m_threadBody(nullptr), m_delayThread(nullptr), mAllowAsyncTransactions(false),
        ...
        std::atomic<bool> mAllowAsyncTransactions;          ///< flag which specifies if async transactions are enabled
