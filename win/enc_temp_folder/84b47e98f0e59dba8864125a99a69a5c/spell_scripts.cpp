        ...
        case SPELL_FUMPING:
        {
            if (uiEffIndex == EFFECT_INDEX_2)
            {
                switch (urand(0, 2))
                {
                    ...
                    case 1:
                    {
                        if (Creature* pSandGnome = pCaster->SummonCreature(NPC_SAND_GNOME, pCreatureTarget->GetPositionX(), pCreatureTarget->GetPositionY(), pCreatureTarget->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 30000))
                            pSandGnome->AI()->AttackStart(pCaster);
                        float x= pCreatureTarget->GetPositionX()+3, y= pCreatureTarget->GetPositionY()+3, z= pCreatureTarget->GetPositionZ();
                        pCaster->UpdateAllowedPositionZ(x, y, z);
                        if (Creature* pSandGnome = pCaster->SummonCreature(NPC_SAND_GNOME, x, y, z, 0.0f, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 30000))
                            pSandGnome->AI()->AttackStart(pCaster);
                        break;
                    }
                    case 2:
                    {
                        if (Creature* pMatureBoneSifter = pCaster->SummonCreature(NPC_MATURE_BONE_SIFTER, pCreatureTarget->GetPositionX(), pCreatureTarget->GetPositionY(), pCreatureTarget->GetPositionZ(), 0.0f, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 30000))
                            pMatureBoneSifter->AI()->AttackStart(pCaster);
                        float x = pCreatureTarget->GetPositionX()+3, y = pCreatureTarget->GetPositionY()+3, z = pCreatureTarget->GetPositionZ();
                        pCaster->UpdateAllowedPositionZ(x, y, z);
                        if (Creature* pMatureBoneSifter = pCaster->SummonCreature(NPC_MATURE_BONE_SIFTER, x, y, z, 0.0f, TEMPSUMMON_TIMED_OOC_OR_DEAD_DESPAWN, 30000))
                            pMatureBoneSifter->AI()->AttackStart(pCaster);
                        break;
                    }
                }
