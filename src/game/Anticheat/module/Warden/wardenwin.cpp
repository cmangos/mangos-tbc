...
namespace
{
static constexpr uint32 sOfsGetText = 0x307200;

static constexpr uint32 sOfsExecute = 0x306C80;

static constexpr uint32 sOfsOpen = 0x257970;
static constexpr uint32 sOfsSize = 0x254080;
static constexpr uint32 sOfsRead = 0x254E00;
static constexpr uint32 sOfsClose = 0x255290;

static constexpr uint32 sOfsTickCount = 0x349850;

static constexpr uint32 sOfsCWorld__enables = 0xDA4510;

static constexpr uint32 sOfsCSimpleTop__m_eventTime = 0xBE10FC;

static constexpr uint32 sOfsg_theGxDevicePtr = 0xD2A15C;
static constexpr uint32 sOfsDevice2 = 0x3864;
static constexpr uint32 sOfsDevice3 = 0x0;
static constexpr uint32 sOfsDevice4 = 0xA8;

static constexpr uint32 sOfsWardenModule = 0xE118D8;
...
void WardenWin::LoadScriptedScans()
{
    ...
    sWardenScanMgr.AddWindowsScan(wardenSysInfo1);
    sWardenScanMgr.AddWindowsScan(wardenSysInfo2);
    ...
    [] (const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!result)
        {
            sLog.outBasic("WARDEN: Failed to read CWorld::enables from account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        {
            sLog.outBasic("WARDEN: CWorld::enables expected 0x%lx prohibited 0x%lx received 0x%lx",
                uint64(Required), uint64(Prohibited), uint64(val));

            return true;
        }

        return false;
    }, "CWorld::enables hack", WinBuild8606));
    ...
    [](const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!buff.read<uint8>())
        {
            sLog.outBasic("WARDEN: Account %u timing check failed to read CSimpleTop::m_eventTime",
        ...
        if (!wardenWin->_session->GetPlayer() || !wardenWin->_session->GetPlayer()->IsInWorld())
        {
            sLog.outDebug("WARDEN: Player not in world. Skipping AFK check for account %u.", session->GetAccountId());
            return false;
        }
        ...
        if (!timeResult)
        {
            ...
            // unfair speed.
            sLog.outDebug("WARDEN: Account %u timing check failed", session->GetAccountId());
            return false;
        }
        ...
        {
            sLog.outBasic("WARDEN: Account: %u Current time: %u Last hardware action: %u (last hardware action in the future)",
        ...
        return false;
    }, 11, 10, "Anti-AFK hack", WinBuild8606));
    ...
    [](const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!result)
        {
            sLog.outBasic("WARDEN: Failed to read EndScene (hook check stage 1) from account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    [endSceneHookCheck1](const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!result)
        {
            sLog.outBasic("WARDEN: Failed to read EndScene (stage 4) from account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    [endSceneLocate4](const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!result)
        {
            sLog.outBasic("WARDEN: Failed to read EndScene (stage 3) from account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    [endSceneLocate3](const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!result)
        {
            sLog.outBasic("WARDEN: Failed to read EndScene (stage 2) from account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
    ...
    [endSceneLocate2](const Warden *warden, ByteBuffer &buff)
    {
        ...
        if (!!result)
        {
            sLog.outBasic("WARDEN: Failed to read g_theGxDevicePtr from account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        ...
        {
            sLog.outBasic("WARDEN: g_theGxDevicePtr is nullptr for account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
            return true;
        }
        ...
        return false;
    }, "EndScene locate stage 1", WinBuild8606 | InitialLogin));
    ...
    Scan::CheckT([] (const Warden *warden, ByteBuffer &buff)
    {
        if (buff.read<uint8>() == WindowsModuleScan::ModuleFound)
        {
            ...
            sLog.outBasic("WARDEN: Proxifier found on account %u ip %s",
                wardenWin->_session->GetAccountId(), wardenWin->_session->GetRemoteAddress().c_str());
        }
