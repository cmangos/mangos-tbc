...
struct npc_corrupted_furbolgAI : public CombatAI
{
    ...
    npc_corrupted_furbolgAI(Creature* creature) : CombatAI(creature, ACTION_MAX)
    {
        ...
        AddCombatAction(ACTION_THUNDERCLAP, true);
        AddCombatAction(ACTION_HEALING_WARD, true);
    }
    ...
    void Reset() override
    {
        ...
        {
            case NPC_BLACKWOOD_WARRIOR: ResetIfNotStarted(ACTION_THUNDERCLAP, GetSubsequentActionTimer(ACTION_THUNDERCLAP)); DoCastSpellIfCan(m_creature, SPELL_BATTLE_STANCE); break;
            case NPC_BLACKWOOD_TOTEMIC: ResetIfNotStarted(ACTION_HEALING_WARD, urand(6, 12) * IN_MILLISECONDS); break;
        }
