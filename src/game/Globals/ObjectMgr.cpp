...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    for (uint32 i = 1; i < sCreatureStorage.GetMaxEntry(); ++i)
    {
        ...
        if (!cInfo->UnitClass || ((getBitmask(cInfo->UnitClass) & CLASSMASK_ALL_CREATURES) == 0))
        {
...
void ObjectMgr::LoadCreatureClassLvlStats()
{
    ...
    do
    {
        ...
        if ((getBitmask(creatureClass) & CLASSMASK_ALL_CREATURES) == 0)
        {
...
void ObjectMgr::LoadCreatureModelInfo()
{
    ...
    for (uint32 race = 1; race < sChrRacesStore.GetNumRows(); ++race)
    {
        ...
        if (!(getBitmask(race) & RACEMASK_ALL_PLAYABLE))
            continue;
...
void ObjectMgr::LoadPlayerInfo()
{
    // Load playercreate
    {
        ...
        do
        {
            ...
            if (!rEntry || !(getBitmask(current_race) & RACEMASK_ALL_PLAYABLE))
            {
            ...
            if (!cEntry || !(getBitmask(current_class) & CLASSMASK_ALL_PLAYABLE))
            {
    ...
    // Load playercreate skills
    {
        ...
        else
        {
            ...
            do
            {
                ...
                {
                    const uint32 raceIndexMask = getBitmask(raceIndex);
                    if (!raceMask || (raceMask & raceIndexMask))
                    {
                        for (uint32 classIndex = CLASS_WARRIOR; classIndex < MAX_CLASSES; ++classIndex)
                        {
                            const uint32 classIndexMask = getBitmask(classIndex);
    ...
    for (int race = 1; race < MAX_RACES; ++race)
    {
        ...
        if (!(getBitmask(race) & RACEMASK_ALL_PLAYABLE) || !sChrRacesStore.LookupEntry(race))
            continue;

        for (int class_ = 1; class_ < MAX_CLASSES; ++class_)
        {
            // skip nonexistent classes
            if (!(getBitmask(class_) & CLASSMASK_ALL_PLAYABLE) || !sChrClassesStore.LookupEntry(class_))
                continue;
