...
struct boss_grobbulusAI : public CombatAI
{
    boss_grobbulusAI(Creature* creature) : CombatAI(creature, GROBBULUS_ACTION_MAX), m_instance(static_cast<ScriptedInstance*>(creature->GetInstanceData()))
    {
        ...
        AddTimerlessCombatAction(GROBBULUS_CHECK_RANGE, true);
    }
    ...
    uint32 GetSubsequentActionTimer(uint32 id)
    {
        switch (id)
        {
            case GROBBULUS_INJECTION: return m_creature->GetHealthPercent() > 30.f ? urand(7, 13) * IN_MILLISECONDS : urand(3, 7) * IN_MILLISECONDS;
    ...
    void ExecuteAction(uint32 action) override
    {
        ...
        switch (action)
        {
            case GROBBULUS_CHECK_RANGE:
            {
                ...
                {
                    ResetIfNotStarted(GROBBULUS_SLIME_STREAM, 3u * IN_MILLISECONDS);
                }
                return;
            }
            case GROBBULUS_SLIME_STREAM:
            {
                if (!m_creature->CanReachWithMeleeAttack(m_creature->GetVictim()))
                {
                    if (DoCastSpellIfCan(m_creature, SPELL_SLIME_STREAM) == CAST_OK)
                        ...
                        ResetTimer(GROBBULUS_SLIME_STREAM, 3u * IN_MILLISECONDS);
                }
                else
                {
