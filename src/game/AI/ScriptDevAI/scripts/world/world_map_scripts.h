...
struct ShadeOfTheHorsemanData
{
    ...
    // TBC only timer section
    bool Update(const uint32 diff); // returns true when event should be started

    uint32 CalculateWorldstateTimerValue() const;

    std::tm m_horsemanTime;
    uint32 m_horsemanTimer;
