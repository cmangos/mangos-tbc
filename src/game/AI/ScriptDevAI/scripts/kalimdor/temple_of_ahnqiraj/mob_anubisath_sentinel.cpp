...
struct npc_anubisath_sentinelAI : public ScriptedAI
{
    ...
    void Aggro(Unit* who) override
    {
        if (!m_myAbility)
        {
            std::shuffle(m_abilities.begin(), m_abilities.end(), std::mt19937(std::random_device()())); // shuffle the abilities, they will be set to the current creature and its siblings
            SetAbility();
